...
account_id = __import__("os").environ["CF_ACCOUNT_ID"]
db_id = __import__("os").environ["D1_DATABASE_ID"]
token = __import__("os").environ["CLOUDFLARE_API_TOKEN"]

sql = Path("infra/d1/schema.sql").read_text(encoding="utf-8")
...
for line in sql.splitlines():
    ...
    if stripped.startswith("--") or stripped == "":
        continue
    if stripped.lower().startswith("pragma journal_mode"):
        continue
    lines.append(line)
...
for i, stmt in enumerate(statements, start=1):
    print(f"Applying schema statement #{i} ...", flush=True)
